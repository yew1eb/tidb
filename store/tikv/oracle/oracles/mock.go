...
// MockOracle is a memory oracle for testing.
...
// Disable stops the oracle. All calls will return errStopped.
...
// SetOffset resets the time offset.
...
// AddOffset increases the time offset.
...
// GetTimestamp returns a ts based on local time and offset, implements Oracle
// interface.
...
// GetTimestampAsync reutrns a future which can take ts from later, implements
// Oracle interface.
func (o *MockOracle) GetTimestampAsync(ctx goctx.Context) oracle.Future {
	return &mockOracleFuture{o, ctx}
}

// IsExpired checks if a timestamp is expired, implements Oracle interface.
...
// Close does nothing, only to implements Oracle interface.
