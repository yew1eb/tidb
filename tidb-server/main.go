...
var (
	...
	addIndexTaskCnt       = flag.Int("task-cnt", 24, "the task number of every batch operation in adding index operation")
	addIndexRowsInPerTask = flag.Int("rows-in-task", 128, "the rows number of every task in adding index operation")
...
func main() {
	...
	ddl.DefaultTaskCnt = *addIndexTaskCnt
	ddl.DefaultTaskHandleCnt = *addIndexRowsInPerTask
